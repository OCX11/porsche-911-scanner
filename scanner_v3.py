#!/usr/bin/env python3
"""
Porsche 911 Scanner v3.0 - Real PCA Mart data, photo thumbnails
10-min cycles, dedup rolling list, Telegram sendPhoto
"""
    ...
    def send_telegram_photo(self, listing: dict) -> bool:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                logger.info(f"✓ Telegram: {year} {model} - ${price:,}")
                return True
            else:
...
[View Full Listing]({url})"""
    ...
    async def run_cycle(self):
        ...
        logger.info(f"✓ Found {len(self.new_listings)} new listings")
        logger.info(f"✓ Rolling list: {len(self.seen_cars)} cars")
        logger.info("=" * 60)

    async def loop(self):
        """Run scanner in continuous loop."""
        logger.info("Starting Porsche 911 Scanner v3.0")
        logger.info(f"PCA Mart + Telegram photo notifications")
